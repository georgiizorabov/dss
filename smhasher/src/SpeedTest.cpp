...
NEVER_INLINE int64_t timehash_memcpy ( const void * key, int len, void *dest)
{
  volatile register int64_t begin,end;
...
double SpeedTest_memcpy ( uint32_t seed, const int trials, const int blocksize, const int align  = 0)
{
  Rand r(seed);
  
  uint8_t * buf = new uint8_t[blocksize + 512];

  uint8_t * dest = new uint8_t[blocksize + 512];
  ...
  for(int itrial = 0; itrial < trials; itrial++)
  {
    ...
    double t = (double)timehash_memcpy(block,blocksize, dest);
...
void TinySpeedTest ( pfHash hash, int hashsize, int keysize, uint32_t seed, bool verbose, double & /*outCycles*/ )
{
  ...
  printf("%8.2f, keysize: %d\n",cycles / keysize, keysize);  
}
...
void TinySpeedTest_memcpy ( int keysize, uint32_t seed )
{
  ...
  double cycles = SpeedTest_memcpy(seed, trials, keysize);
